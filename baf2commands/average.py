"""
Compute borrowings for subsets.
"""
...
def run(args):
    ...
    families = {wl[idx, 'doculect']: wl[idx, 'family'] for idx in wl}
    ...
    for family, sharedi in bangime.items():
        print('{0:30} | {1:.2f}'.format(family, sum([x[1] for x in sharedi])/len(sharedi)))

    print('')
    for language, sharedi in dogon.items():
        print('{0:30} | {1:.2f}'.format(language, sum([x[1] for x in sharedi])/len(sharedi)))
    ...
    with open(ds.dir / "analysis" / 'relations.md', 'w') as f:
        current_family = ''
